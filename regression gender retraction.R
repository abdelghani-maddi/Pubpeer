...
bdd_disc_reg_ss_notices2$Gtype2_rec <- bdd_disc_reg_ss_notices2$Gtype2_rec %>%
  fct_relevel(
    "Man alone", "Women only" , "Men-Women | M first", "Men-Women | W first",
...
var_label(bdd_disc_reg_ss_notices2$nb_aut) <- "# Authors"


# Transform nb_aut into a categorical variable
bdd_disc_reg_ss_notices2$nb_aut_cat <- cut(bdd_disc_reg_ss_notices2$nb_aut,
                                           breaks = c(0, 1, 5, 10, 200),  # Intervals
                                           labels = c("Alone (1)", "Small team (2-5)", "Medium team (6-10)", "Large team (>10)"),
                                           right = TRUE)  # Includes upper bound
###
res <- glm(is_retracted ~ Gtype2_rec + is_oa + log(1+cited_by_count) + nb_aut_cat +
